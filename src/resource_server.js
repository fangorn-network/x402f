...
if (!rpcUrl)
    throw new Error("CHAIN_RPC_URL required");
const jwt = process.env.PINATA_JWT;
if (!jwt)
    throw new Error("PINATA_JWT required");
const gateway = process.env.PINATA_GATEWAY;
if (!gateway)
    throw new Error("PINATA_GATEWAY required");
const delegatorAccount = privateKeyToAccount(getEnv("DELEGATOR_ETH_PRIVATE_KEY"));
        ...
        accepts: [
            {
                scheme: "exact",
                price: async (request) => {
                    // const { vaultId, tag } = request.body;
                    return "$0.00001";
                },
